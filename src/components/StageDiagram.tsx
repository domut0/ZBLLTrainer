...
export interface StageDiagramProps {
  /** 28 colour letters (21 LL + 7 slot). */
...
/**
 * A stage diagram for F2L-stage algorithm sets (LXS, etc.): the 21 last-layer
 * stickers plus the 7 FR/DR slot stickers. Sibling component to `LLDiagram`.
 */
export function StageDiagram({
  ...
  if (fault !== null) {
    if (import.meta.env.DEV) {
      throw new Error(`StageDiagram: malformed facelet string — ${fault}`)
    }
    console.error(`StageDiagram: malformed facelet string — ${fault}`)
    return null
  }

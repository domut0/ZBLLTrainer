...
/** Expected sticker count for an EO diagram (21 LL + 7 FR/DR slot). */
export const EO_FACELET_COUNT = 28
 ...
 * Matches StageDiagram's 28 sticker positions.
 */
const STICKERS: readonly Sticker[] = (() => {
  ...
  // 21-27: FR/DR slot stickers, aligned under F and R bars
...
function faultIn(facelets: FaceletString): string | null {
  ...
  if (facelets.length !== EO_FACELET_COUNT) {
    return `expected ${EO_FACELET_COUNT} characters, got ${facelets.length} (${JSON.stringify(facelets)})`
  }
  for (let i = 0; i < EO_FACELET_COUNT; i += 1) {
    const ch = facelets[i]
...
export interface EoDiagramProps {
  /** 28 colour letters (21 LL + 7 slot), using '0' (oriented) and '1' (misoriented). */
...
/**
 * An explicit Edge Orientation diagram component. Shows the orientation of the 6
 * relevant edges (UF, UR, UB, UL, FR, DR) explicitly: oriented edges ('0') are drawn
 * green, misoriented edges ('1') are drawn red with a distinct white flip marker.
 * Every other sticker is '?' grey.
 */
export function EoDiagram({
  facelets,
  className,
  label = 'EO diagram',
}: EoDiagramProps) {
  ...
  if (fault !== null) {
    if (import.meta.env.DEV) {
      throw new Error(`EoDiagram: malformed facelet string — ${fault}`)
    }
    console.error(`EoDiagram: malformed facelet string — ${fault}`)
    return null
  }
  ...
  return (
    ...
    >
      <title>{label}</title>
      {STICKERS.map((s) => {
        const val = facelets[s.index]
        const isFlipped = val === '1'
        const cx = s.x + s.width / 2
        const cy = s.y + s.height / 2
        const r = Math.min(s.width, s.height) * 0.22
        ...
        return (
          <g key={s.index}>
            <rect
              ...
              rx={s.rx}
              ry={s.rx}
              fill={FACELET_FILL[val as FaceletColour]}
              stroke="#09090b"
              strokeWidth={4}
            />
            {isFlipped && (
              <circle
                data-flip-marker={s.index}
                cx={cx}
                cy={cy}
                r={r}
                fill="#ffffff"
